 ...
 */
/*
function taskbrew_form_alter(&$form, $form_state, $form_id) {
  if ($form_id == 'install_configure_form' && !defined('DRUSH_BASE_PATH')) {
    ...
    $form['#validate'][] = 'taskbrew_password_validate';
    $form['#submit'][] = 'taskbrew_password_submit';  
  }
}
*/
...
*/
function taskbrew_admin_paths_alter(&$paths) {  
 ...
 */
function taskbrew_password_validate($form, &$form_state) {
 ...
 */
function taskbrew_password_submit($form, &$form_state) {
  global $user;
 ...
 */
function taskbrew_apps_servers_info() {
 $info =  drupal_parse_info_file(dirname(__file__) . '/taskbrew.info');
 return array(
   'taskbrew' => array(
     'title' => 'taskbrew',
     'description' => "Apps for the taskbrew distribution",
     'manifest' => 'http://appserver.taskbrewapp.com/app/query',
     'profile' => 'taskbrew',
 ...
 */
function taskbrew_init() {
 $cache = cache_get("taskbrew_info");
 if (isset($cache->data)) {
   $data = $cache->data;
 }
 else {
   $info =  drupal_parse_info_file(dirname(__file__) . '/taskbrew.info');
   $data = array("profile" => "taskbrew", "profile_version" => $info['version']);
   cache_set("taskbrew_info", $data);
 }
 ...
 */
function taskbrew_form_install_configure_form_alter(&$form, &$form_state) {
  $form['site_information']['site_name']['#default_value'] = 'taskbrew'; 
